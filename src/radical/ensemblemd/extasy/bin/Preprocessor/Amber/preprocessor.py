...
def Preprocessing(Kconfig,umgr,cycle):
    ...
    if (cycle==0):
        ...
        cudesc.input_staging = ['%s/%s'%(mdshort_loc,mdshortfile),'%s/%s'%(min_loc,minfile),'%s/%s'%(crd_loc,crdfile),'%s/%s'%(top_loc,topfile),'%s/cocoUi.py'%os.path.dirname(os.path.realpath(__file__))]

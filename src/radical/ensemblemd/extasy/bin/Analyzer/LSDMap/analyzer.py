...
def Analyzer(umgr,RPconfig,Kconfig,cycle):
    ...
    lsdm.pre_exec.append('module load gromacs')
    lsdm.pre_exec.append('echo 2 | trjconv -f %s -s %s -o tmpha.gro &>/dev/null'%(Kconfig.tmp_grofile,Kconfig.tmp_grofile))
    ...
    os.system('mv %s %s_%s'%(Kconfig.outgrofile_name,cycle,Kconfig.input_gro))

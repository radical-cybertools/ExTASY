...
def Analyzer(umgr,RPconfig,Kconfig,cycle):
    ...
    mdtd.kernel="CoCo"
    mdtd.arguments = ['-l','-c','python pycoco.py --grid %s --dims %s --frontpoints %s --topfile %s --mdfile *.mdcrd --output pentaopt%s'%(Kconfig.grid,Kconfig.dims,Kconfig.frontpoints,Kconfig.topfile,cycle) ]

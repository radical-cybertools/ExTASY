...
def Simulator(umgr,RPconfig,Kconfig,cycle,paths):
    ...
    for i in range(0, Kconfig.num_CUs):
        ...
        if Kconfig.ndx_file is not None:
            ...
            ndx_opts = ''
        mdtd.arguments = ['-l','-c','export grompp_options="%s" mdrun_options="%s" ndxfile="%s" && . run.sh %s start.gro %s out.gro %s' % (grompp_opts,mdrun_opts,ndxfile_name,grompp_name,topol_name,Kconfig.num_cores_per_sim_cu)]
        ...
        inputs = ['cp %s/run.sh .'%paths[0],'cp %s/temp/start%s.gro start.gro'%(paths[cycle],i),'cp %s/%s .'%(paths[0],grompp_name),'cp %s/%s .'%(paths[0],topol_name)]
